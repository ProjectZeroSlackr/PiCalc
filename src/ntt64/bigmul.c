...
if (Cache==-2) /* In memory convolution of both nums */
  {ModInt *FFTNum2=FFTNum+NTTLen;
   DumpDebug("Cm...");
   for (x=0;x<NTTLen;x++) FFTNum[x]=ModMul(FFTNum[x],FFTNum2[x]);
  }
else if (Cache==-1)  /* In memory self convolution. */
  {
   DumpDebug("C");
   for (x=0;x<NTTLen;x++) FFTNum[x]=ModMul(FFTNum[x],FFTNum[x]);
  }
#ifdef VIRTUAL_CACHE
  ...
  {ModInt *Num1=FFTNum;
   ...
   DumpDebug("C%d...",Cache);
   for (x=0;x<NTTLen;x++) Num1[x]=ModMul(Num1[x],Num2[x]);
  }
#else
  ...
  {FILE *f;ModInt *Num1=FFTNum;
   ...
   while (L2)
     {size_t Sz;
      ...
      StopTimer(DiskIOTime);
      for (x=0;x<Sz;x++) Num1[x]=ModMul(Num1[x],Num2[x]);
      Num1+=Sz;
      L2-=Sz;
     }
...
/* Can we do both numbers in memory? */
  ...
  {INT32 *P;
   UINT64 Carry, Pyramid;
   P=(INT32*)FFTNum;
   Carry = 0;
// HARDWIRED
   for (x = 0; x < NTTLen; x+=2)
      {INT32 d1,d2;
        Pyramid = ModMul(FFTNum[x],MulInv) + Carry;
        Carry   = Pyramid / 10000;
        d1      = Pyramid % 10000;

        Pyramid = ModMul(FFTNum[x+1],MulInv) + Carry;
        Carry   = Pyramid / 10000;
        d2      = Pyramid % 10000;
...
/*
   for (x = 0; x < NumLen2; x++)
      {
        Pyramid = ModMul(FFTNum[x],MulInv) + Carry;
        Carry   = Pyramid / BaseVar;
        P[x]    = Pyramid % BaseVar;
      }
*/
   StopTimer(CarryTime);
   for (x=0;x<NumLen;x++) {INT32 t=P[x];P[x]=P[NumLen2-x-1];P[NumLen2-x-1]=t;}
   if (Scale) ScaleCarry=BlockMulBy(P,P,Scale,0,Min(ProdLen,NumLen2));
   WriteBufIntoNum(P,Prod,Min(NumLen2,ProdLen));
  }
return ScaleCarry;
}
