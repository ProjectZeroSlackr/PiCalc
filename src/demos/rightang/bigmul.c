...
#endif
//printf("\n");
//for (x=0;x<FFTLen2;x+=2)  printf("x=%ld %30.15f %30.15f\n",x,FFTNum[x]*inv,FFTNum[x+1]*inv);
//exit(0);
...
#if 1
{
   ...
   Carry = 0.0;
   for (x = 0; x < FFTLen2; x+=4)
      {int D1,D2;
        Pyramid = FLOOR(FFTNum[x] * inv + Carry + 0.5);
        Carry = FLOOR(Pyramid / 10000.0);
        D1 = (Pyramid - Carry * 10000.0);
        Pyramid = FLOOR(FFTNum[x+2] * inv + Carry + 0.5);
        Carry = FLOOR(Pyramid / 10000.0);
        D2 = (Pyramid - Carry * 10000.0);

        ProdNdx-=1;ProdPos-=1;
        ProdBuf[ProdNdx]= D2*10000+D1;
        if (ProdNdx==0)
          {size_t z;
...
//for (x=0;x<NumLen2;x++) printf("x=%ld %d\n",x,Prod[x]);
}
#endif
